...
@Component
public class CustomAuthSuccessHandler extends SimpleUrlAuthenticationSuccessHandler { // 인증 성공시(로그인 성공)의 동작을 컨트롤
    // 사용자가 인증되지 않은 상태에서 접근하려고 했던 요청을 기억 -> 인증 성공하면 리다이렉트
    private final RequestCache requestCache = new HttpSessionRequestCache();
    // 인증 성공 시 다른 URL로 보내는 것
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
        ...
        clearSession(request); // 로그인 실패 시에 남아있는 에러 정보를 세션에서 제거하기 위해
        ...
        if (prevPage != null) {
            request.getSession().removeAttribute("prevPage"); // 사용자가 로그인 하기 전의 페이지 => prevPage로 세션에 저장 되어 있는 경우 확인하고 삭제
        }
        // 기본 URI
        String url = "/";
        /**
