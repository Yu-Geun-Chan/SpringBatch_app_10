...
@Configuration
@EnableWebSecurity // 웹 보안 활성화 -> Spring Security 기본 설정 사용하도록
        ...
        http
                .formLogin(
                        formLogin -> formLogin
                                ...
                                .loginProcessingUrl("/member/login") // 로그인 처리(POST) URL
                                .successHandler(authenticationSuccessHandler)
                                .failureHandler(authenticationFailureHandler)
                )
                .logout(
                        logout -> logout.logoutUrl("/member/logout") // 로그아웃 처리 URL
                );
        return http.build();
    }
}
